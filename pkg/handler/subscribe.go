...
func SubscribeProxy(c *gin.Context) {
	...
	if subscribeGroup == "" && len(user.SubscribeGroups) > 0 {
		log.Printf("Using default subscribe group: %s for user: %s", user.SubscribeGroups[0].Name, user.Name)
		subscribeGroup = user.SubscribeGroups[0].Name
	}
